  ...
  const emailToggle = document.querySelector('.email-toggle');
    const emailMenu = document.querySelector('.email-dropdown-menu');
    if (emailToggle && emailMenu) {
      emailToggle.addEventListener('click', () => emailMenu.classList.toggle('open'));
      document.addEventListener('click', (e) => {
        if (!e.target.closest('.email-dropdown')) emailMenu.classList.remove('open');
      });
    }
